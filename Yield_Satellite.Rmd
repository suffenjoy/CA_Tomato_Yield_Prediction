...
l578_rr_yield <- base::merge(l578_rr_df, yield_rr, by = c("Year","FieldID"))
...
#merge with yield 
l578_rr_month_yield <- merge(l578_rr_month, yield_rr, by = c("Year","FieldID"))
...
names(l578_rr_month_yield)[8] <- "Yield"
...
head(l578_rr_month)
dim(l578_rr_month_yield) #176,92


a <- cor(l578_rr_month_yield$yield, l578_rr_month_yield[,c(9:92)])
View(t(a))
dim(l578_rr_month_yield) #176,92
...
b <- cor(l578_county_monthweighted_yield$Yield, l578_county_monthweighted_yield[,c(11:94)])
View(t(b))
...
#convert long to wide 
weather_davis_wide <- tidyr::pivot_wider(weather_davis, names_from = Year, values_from = MaxTemp:vs)
...
weather_davis_wide <- merge(weather_davis_wide, extreme_rr_wide, by = "Year")
weather_county_wide <- merge(daymet_county_wide, extreme_county_wide, by = c("County","Year"))
